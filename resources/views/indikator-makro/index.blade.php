    ...
    <div class="fi-page mt-2">
        ...
        <div class="fi-header fade-in-up">
            ...
            <div class="fi-actions">
                @if($selectedIndikator)
                    <button type="button"
                        class="btn btn-success rounded-pill px-3 py-1 shadow-sm d-inline-flex align-items-center gap-1"
                        id="btn-export-modal" data-bs-toggle="modal" data-bs-target="#exportModal"
                ...
                @auth
                    @if(auth()->user()->status === 'active' && auth()->user()->kabupaten->kode_kab === '6100')
                        <a href="{{ route('indikator-makro.input-nilai') }}" class="fi-btn-action"
                            style="background: #2ecc71; box-shadow: 0 2px 8px rgba(46, 204, 113, .3); margin-left: 0.5rem;">
                ...
                <form action="{{ route('indikator-makro.index') }}" method="GET" class="row g-3" id="filter-form">
                    ...
                    <div class="col-md-12 mb-2">
                        <div class="d-flex justify-content-between align-items-center mb-1">
                            <label class="form-label text-muted small fw-bold text-uppercase mb-0">Pilih Indikator
                                Makro</label>
                            <button type="button" class="btn btn-sm btn-outline-warning rounded-pill px-3 py-1"
                                id="btn-open-katalog" data-bs-toggle="modal" data-bs-target="#katalogModal"
                        ...
                        <div class="position-relative" id="indikator-search-wrapper">
                            <div class="input-group">
                                ...
                                <input type="text" id="indikator-search-input"
                                    class="form-control border-start-0 ps-1 bg-light" placeholder="Cari Indikator Makro..."
                                    value="{{ $selectedIndikator ? $selectedIndikator->nama_indikator : '' }}"
                                    autocomplete="off" required>
                                <button class="btn btn-outline-secondary border-start-0" type="button" id="btn-clear-search"
                                    title="Bersihkan Pilihan" style="{{ $selectedIndikator ? '' : 'display: none;' }}">
                    ...
                    @if($selectedIndikator)
                        ...
                        <div class="col-md-12 mb-2">
                            <div class="d-flex justify-content-between align-items-center mb-1 flex-wrap gap-2">
                                <label class="form-label text-muted small fw-bold text-uppercase mb-0">Filter Tahun / Periode
                                    (Tabel & Grafik)</label>
                                <div class="d-flex gap-2">
                                    <button type="button" class="btn btn-xs btn-outline-primary py-0 px-2"
                                        id="btn-filter-tahun-all" style="font-size: 0.75rem;">Pilih Semua</button>
                                    <button type="button" class="btn btn-xs btn-outline-secondary py-0 px-2"
                                        id="btn-filter-tahun-none" style="font-size: 0.75rem;">Hapus Semua</button>
                                </div>
                            </div>
                            <div class="d-flex flex-wrap gap-3 p-2 bg-light rounded border px-3">
                                @foreach($allPeriodes as $p)
                                    <div class="form-check">
                                        <input class="form-check-input year-filter-checkbox" type="checkbox" name="tahun[]"
                                            value="{{ $p->tahun }}" id="year_{{ $p->tahun }}" {{ in_array($p->tahun, $selectedYears) ? 'checked' : '' }} onchange="this.form.submit()">
                                        <label class="form-check-label small fw-semibold"
                                            for="year_{{ $p->tahun }}">{{ $p->tahun }}</label>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                        ...
                        @if(!$isNoneOnly)
                            <div class="col-md-12 mb-2">
                                <div class="d-flex justify-content-between align-items-center mb-1 flex-wrap gap-2">
                                    <label class="form-label text-muted small fw-bold text-uppercase mb-0">Filter Dimensi /
                                        Klasifikasi (Tabel)</label>
                                    <div class="d-flex gap-2">
                                        <button type="button" class="btn btn-xs btn-outline-primary py-0 px-2"
                                            id="btn-filter-dim-all" style="font-size: 0.75rem;">Pilih Semua</button>
                                        <button type="button" class="btn btn-xs btn-outline-secondary py-0 px-2"
                                            id="btn-filter-dim-none" style="font-size: 0.75rem;">Hapus Semua</button>
                                    </div>
                                </div>
                                <div class="d-flex flex-wrap gap-3 p-2 bg-light rounded border px-3">
                                    @foreach($indikatorDimensis as $indDim)
                                        <div class="form-check">
                                            <input class="form-check-input dimensi-filter-checkbox" type="checkbox"
                                                name="indikator_dimensi_ids[]" value="{{ $indDim->id }}" id="dim_{{ $indDim->id }}" {{ in_array($indDim->id, $selectedDimensiIds) ? 'checked' : '' }}
                        ...
                        <div class="col-md-12">
                            <h6 class="fw-bold small text-muted text-uppercase mb-2"><i
                                    class="fas fa-chart-bar me-1 text-primary"></i> Pengaturan Filter Grafik</h6>
                        </div>
                        ...
                        @if(!$isNoneOnly)
                            <div class="col-md-12 mb-2">
                                ...
                                <div class="d-flex gap-4 flex-wrap">
                                    <div class="form-check">
                                        <input class="form-check-input chart-mode-radio" type="radio" name="chart_mode"
                                            id="mode_compare_regions" value="compare_regions" checked>
                                        <label class="form-check-label small fw-bold text-dark" style="cursor: pointer;"
                                            for="mode_compare_regions">Bandingkan Wilayah (Pilih 1 Dimensi, Sumbu X:
                                            Wilayah)</label>
                                    </div>
                                    <div class="form-check">
                                        <input class="form-check-input chart-mode-radio" type="radio" name="chart_mode"
                                            id="mode_compare_dimensions" value="compare_dimensions">
                                        <label class="form-check-label small fw-bold text-dark" style="cursor: pointer;"
                                            for="mode_compare_dimensions">Bandingkan Dimensi (Pilih 1 Wilayah, Sumbu X:
                                            Dimensi)</label>
                                    </div>
                                    <div class="form-check">
                                        <input class="form-check-input chart-mode-radio" type="radio" name="chart_mode"
                                            id="mode_compare_all" value="compare_all">
                                        <label class="form-check-label small fw-bold text-dark" style="cursor: pointer;"
                                            for="mode_compare_all">Bandingkan Wilayah & Dimensi (Sumbu X: Wilayah, Datasets:
                                            Dimensi)</label>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-12 mb-2" id="single-dimensi-container">
                                ...
                                <select id="chart-single-dimensi" class="form-select bg-light border-0 py-2"
                            ...
                            <div class="col-md-12 mb-2 d-none" id="multi-dimensi-container">
                                <label class="form-label text-muted small fw-bold text-uppercase">Pilih Dimensi Grafik (Bisa
                                    Banyak)</label>
                                <div class="d-flex flex-wrap gap-3 p-2 bg-light rounded border px-3">
                                    @foreach($indikatorDimensis as $indDim)
                                        <div class="form-check">
                                            <input class="form-check-input chart-dim-checkbox" type="checkbox" value="{{ $indDim->id }}"
                                                id="chk_chart_dim_{{ $indDim->id }}" checked>
                        ...
                        <div class="col-md-12 mb-2" id="multi-region-container">
                            <div class="d-flex justify-content-between align-items-center mb-1 flex-wrap gap-2">
                                <label class="form-label text-muted small fw-bold text-uppercase mb-0">Pilih Wilayah Grafik
                                    (Bisa Banyak)</label>
                                <div class="d-flex gap-2">
                                    <button type="button" class="btn btn-xs btn-outline-primary py-0 px-2"
                                        id="btn-chart-select-all" style="font-size: 0.75rem;">Pilih Semua</button>
                                    <button type="button" class="btn btn-xs btn-outline-secondary py-0 px-2"
                                        id="btn-chart-deselect-all" style="font-size: 0.75rem;">Hapus Semua</button>
                                </div>
                            </div>
                            <div class="d-flex flex-wrap gap-3 p-2 bg-light rounded border px-3">
                                @foreach($kabupatens as $kab)
                                    ...
                                    @if($kabChart)
                                        <div class="form-check">
                                            <input class="form-check-input region-chart-checkbox" type="checkbox" value="{{ $kab->id }}"
                                                id="chk_region_{{ $kab->id }}" {{ ($kab->kode_kab === '6100' || $kab->kode_kab === '1') ? 'checked' : '' }}>
                        ...
                        <div class="col-md-12 mb-2 d-none" id="single-region-container">
                            ...
                            <select id="chart-single-region" class="form-select bg-light border-0 py-2"
                                style="font-size: 0.9rem;">
                                @foreach($kabupatens as $kab)
                                    @php
                                        $kabChart = collect($chartData)->first(fn($item) => $item['id'] === $kab->id);
                            ...
                            <div>
                                ...
                                <p class="text-muted small mb-0">
                                    ...
                                    @if(!empty($selectedYears))
                                        Tahun
                                        {{ count($selectedYears) === 1 ? $selectedYears[0] : min($selectedYears) . ' - ' . max($selectedYears) }}
                                    @endif
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="table-bps-scroll">
                            <table class="table-bps align-middle {{ $isNoneOnly ? 'table-none-only' : 'table-multi-dim' }}">
                                ...
                                <tbody>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="{{ ($periodes->count() * $selectedDimensis->count()) + 1 }}"
        ...
        @else
            ...
            <div class="card card-custom border-0 shadow-sm py-5 px-4 text-center">
                ...
                <p class="text-muted small">Silakan tambahkan atau aktifkan Indikator Makro terlebih dahulu di halaman kelola.
                </p>
            </div>
        @endif
            ...
            <div class="modal-dialog modal-lg modal-dialog-scrollable">
                <div class="modal-content border-0 shadow" style="border-radius: 16px;">
                    <div class="modal-header border-bottom-0 pb-0">
                        <div>
                            <h5 class="modal-title fw-bold text-dark" id="katalogModalLabel"
                                style="color: var(--fi-primary) !important;">
                                <i class="fas fa-list-ul me-2"></i> Katalog Indikator Makro
                            </h5>
                            <p class="text-muted small mb-0">Silakan pilih indikator makro berdasarkan bidang di bawah ini
                            </p>
                        </div>
            ...
            <div class="modal fade" id="exportModal" tabindex="-1" aria-labelledby="exportModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-lg modal-dialog-scrollable">
                    <div class="modal-content border-0 shadow" style="border-radius: 16px;">
                        <div class="modal-header border-bottom-0 pb-0">
                            <div>
                                <h5 class="modal-title fw-bold text-dark" id="exportModalLabel"
                                    style="color: var(--fi-primary) !important;">
                                    <i class="fas fa-file-excel me-2"></i> Ekspor Data Indikator Makro
                                </h5>
                                <p class="text-muted small mb-0">Silakan pilih parameter data yang ingin diekspor ke Microsoft
                                    Excel</p>
                            </div>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body p-4 pt-3">
                            ...
                            <div class="mb-3">
                                <div class="d-flex justify-content-between align-items-center mb-1 flex-wrap gap-2">
                                    <label class="form-label small fw-bold text-muted text-uppercase mb-0">1. Pilih Bidang
                                        Indikator</label>
                                    <div class="d-flex gap-2">
                                        <button type="button" class="btn btn-xs btn-outline-primary py-0 px-2"
                                            id="btn-export-bidang-all" style="font-size: 0.7rem;">Pilih Semua</button>
                                        <button type="button" class="btn btn-xs btn-outline-secondary py-0 px-2"
                                            id="btn-export-bidang-none" style="font-size: 0.7rem;">Hapus Semua</button>
                                    </div>
                                </div>
                                <div class="d-flex flex-wrap gap-3 p-2 bg-light rounded border px-3"
                                    id="export-bidang-container">
                            ...
                            <div class="mb-3">
                                <div class="d-flex justify-content-between align-items-center mb-1 flex-wrap gap-2">
                                    <label class="form-label small fw-bold text-muted text-uppercase mb-0">2. Pilih Indikator
                                        Makro</label>
                                    <div class="d-flex gap-2">
                                        <button type="button" class="btn btn-xs btn-outline-primary py-0 px-2"
                                            id="btn-export-ind-all" style="font-size: 0.7rem;">Pilih Semua</button>
                                        <button type="button" class="btn btn-xs btn-outline-secondary py-0 px-2"
                                            id="btn-export-ind-none" style="font-size: 0.7rem;">Hapus Semua</button>
                                    </div>
                                </div>
                                <div class="mb-2">
                                    <div class="input-group input-group-sm shadow-sm"
                                        ...
                                        <input type="text" id="search-export-indikator" class="form-control border-start-0"
                                            placeholder="Cari nama indikator makro..." style="font-size: 0.85rem;">
                                    </div>
                                </div>
                                <div class="d-flex flex-column gap-2 p-2 bg-light rounded border px-3"
                                    style="max-height: 200px; overflow-y: auto;" id="export-indikator-container">
                                    {{-- Dynamically loaded via AJAX --}}
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                <div class="d-flex justify-content-between align-items-center mb-1 flex-wrap gap-2">
                                    <label class="form-label small fw-bold text-muted text-uppercase mb-0">3. Pilih Tahun /
                                        Periode</label>
                                    <div class="d-flex gap-2">
                                        <button type="button" class="btn btn-xs btn-outline-primary py-0 px-2"
                                            id="btn-export-tahun-all" style="font-size: 0.7rem;">Pilih Semua</button>
                                        <button type="button" class="btn btn-xs btn-outline-secondary py-0 px-2"
                                            id="btn-export-tahun-none" style="font-size: 0.7rem;">Hapus Semua</button>
                                    </div>
                                </div>
                                <div class="d-flex flex-wrap gap-3 p-2 bg-light rounded border px-3">
                                    @foreach($allPeriodes as $p)
                                        <div class="form-check">
                                            <input class="form-check-input export-tahun-checkbox" type="checkbox"
                                                value="{{ $p->tahun }}" id="chk_export_year_{{ $p->tahun }}">
                                            <label class="form-check-label small fw-semibold"
                                                for="chk_export_year_{{ $p->tahun }}">{{ $p->tahun }}</label>
                                        </div>
                                    @endforeach
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                <div class="d-flex justify-content-between align-items-center mb-1 flex-wrap gap-2">
                                    <label class="form-label small fw-bold text-muted text-uppercase mb-0">4. Pilih Wilayah /
                                        Kabupaten</label>
                                    <div class="d-flex gap-2">
                                        <button type="button" class="btn btn-xs btn-outline-primary py-0 px-2"
                                            id="btn-export-kab-all" style="font-size: 0.7rem;">Pilih Semua</button>
                                        <button type="button" class="btn btn-xs btn-outline-secondary py-0 px-2"
                                            id="btn-export-kab-none" style="font-size: 0.7rem;">Hapus Semua</button>
                                    </div>
                                </div>
                                <div class="d-flex flex-wrap gap-3 p-2 bg-light rounded border px-3"
                                    ...
                                    @foreach($kabupatens as $kab)
                                        <div class="form-check">
                                            <input class="form-check-input export-kab-checkbox" type="checkbox"
                                                value="{{ $kab->id }}" id="chk_export_kab_{{ $kab->id }}">
                            ...
                            {{-- Estimasi & Ringkasan --}}
                            <div class="alert alert-warning border-0 p-3 rounded-3 mt-4 mb-0"
                                ...
                                <div class="d-flex align-items-start">
                                    ...
                                    <div class="w-100">
                                        ...
                                        <div class="row g-2 small">
                                            <div class="col-6">
                                                Total Data Sel: <span id="export-total-cells" class="fw-bold text-dark">0</span>
                                                sel
                                            </div>
                                            <div class="col-6">
                                                Estimasi Waktu Ekspor: <span id="export-est-time" class="fw-bold text-success">2
                                                    detik</span>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="modal-footer border-top-0 pt-0">
                            ...
                            <button type="button" id="btn-submit-export"
                                class="btn btn-success rounded-pill px-4 text-white fw-bold">
